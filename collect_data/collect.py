...
def parse_args():
    ...
    parser.add_argument("--timeout", type=int, default=550, help="FinMind 每小時可以傳送的 request 數量")
...
def saveHistory(stockList, start_date, timeout, url, token, conn, cur, subset):
    ...
    for cnt, stock in enumerate(stockList):
        ...
        # send request
        parameter = {"data_id": str(stock).strip(), "dataset": "TaiwanStockPrice", "start_date": start_date, "token": token, 'end_date': end_date}
        ...
        # 計算 MACD 黃金交叉
        df["macd_gc"] = (df["DIF"].shift(1) < df["MACD"].shift(1)) & (df["DIF"] > df["MACD"])
        ...
        df[["macd_gc", "his", "kd_gc", "volume_emerge", "spread_emerge", "volume_emerge_large"]] = \
        df[["macd_gc", "his", "kd_gc", "volume_emerge", "spread_emerge", "volume_emerge_large"]].astype(bool)

        momentum_indices = df[
            (((df["macd_gc"] & df["his"]) | df["kd_gc"]) & ((df["volume_emerge"]) | (df["spread_emerge"]))) 
            | (df["volume_emerge_large"])
        ].index

        # 建立新的 DataFrame 來存放黃金交叉的時間點
        m_df = df.loc[momentum_indices, ["date", "stock_id", "close", "Trading_Volume", "macd_gc", 'his', "kd_gc", 'volume_emerge', 'spread_emerge', 'volume_emerge_large']]
...
def main():
    ...
    saveHistory(stockList, opt.start_date, opt.timeout, url, token, conn, cur, opt.subset)
